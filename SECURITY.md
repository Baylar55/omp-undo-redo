...
The store root and its `repos/` directory are therefore created owner-only (`0700`, POSIX; Windows uses the inherited ACL) and each private repository sets `core.sharedRepository=0600`. Private repositories created before v1.6.2 kept the process umask's modes: on a shared host, remove `<storeRoot>/repos` once so it is recreated hardened.
